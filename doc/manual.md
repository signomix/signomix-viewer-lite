# Podręcznik Użytkownika: Signomix Viewer Lite

*Uwaga: W miarę dalszego rozwoju aplikacji i dodawania nowych funkcji, zaleca się podział tego dokumentu na osobne pliki (np. `installation.md`, `usage.md`, `troubleshooting.md` oraz `security.md`) w celu zachowania czytelności.*

---

## 1. Wprowadzenie

**Signomix Viewer Lite** to lekka, bezpieczna aplikacja typu PWA (Progressive Web App) służąca do przeglądania pulpitów (dashboardów) z platformy **Signomix IoT**. Aplikacja została zaprojektowana z myślą o urządzeniach mobilnych oraz komputerach osobistych, ograniczając do minimum przesyłanie zbędnych danych.
...
Jako aplikacja PWA, Signomix Viewer Lite nie wymaga pobierania ze sklepów z aplikacjami (Google Play / App Store). Działa bezpośrednio w przeglądarce, ale może zostać "zainstalowana" w systemie, zachowując się jak natywna aplikacja.
...
Możesz również wywołać konkretny pulpit podając jego ID w adresie: `https://twojadomena.com/?did=MOJ_DASHBOARD`.
...
Z górnego menu możesz wybrać opcję **"Wybierz pulpit"**. Wyświetli się lista rozwijana zawierająca wszystkie przypisane do Twojego konta dashboardy.
...
Jeśli administrator wdroży nową wersję na serwerze, na dole ekranu automatycznie pojawi się baner **"Dostępna jest nowa wersja aplikacji PWA!"**. Wystarczy kliknąć "Zaktualizuj teraz", a aplikacja bezszelestnie zaktualizuje się do najnowszej wersji.
...
* **"Błędny klucz aplikacji lub problem z serwerem"**: Wprowadzony `App Key` jest niepoprawny lub stracił ważność. Spróbuj zalogować się ponownie z poprawnym kluczem z platformy Signomix.
...
Signomix Viewer Lite został zaprojektowany z zachowaniem nowoczesnych standardów bezpieczeństwa:
...
2. **Ochrona klucza API**: Podany `App Key` **nigdy** nie jest wysyłany jako część adresu URL (co narażałoby go na wyciek w logach serwera). Jest on przesyłany wyłącznie i bezpiecznie w zaszyfrowanych nagłówkach HTTP (`Authentication`).
3. **Lokalne przechowywanie**: Klucz i preferencje są zapisywane w bezpiecznym magazynie przeglądarki (`localStorage`). Nie są one współdzielone między różnymi witrynami. W przypadku współdzielenia urządzenia, zaleca się "Wylogowanie" (podanie pustego/błędnego klucza) lub wyczyszczenie danych przeglądarki.
